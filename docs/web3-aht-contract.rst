...
The ``web3.aht.Contract`` object makes it easy to interact with smart contracts on the bowhead blockchain.
...
.. code-block:: javascript
    var Contract = require('web3-aht-contract');
...
3. ``options`` - ``Object`` (optional): The options of the contract. Some are used as fallbacks for calls and transactions:
    ...
    * ``gasPrice`` - ``String``: The gas price in cell to use for transactions.
...
.. code-block:: javascript
    ...
    var myContract = new web3.aht.Contract([...], '0xde0B295669a9FD93d5F28D9Ec85E40f4cb697BAe', {
        from: '0x1234567890123456789012345678901234567891', // default from address
        gasPrice: '20000000000' // default gas price in cell, 20 gwei in this case
    });
...
.. code-block:: javascript
    ...
    myContract.options.gasPrice = '20000000000000'; // default gas price in cell
...
The :ref:`json interface <glossary-json-interface>` object derived from the `ABI <https://github.com/bowhead/wiki/wiki/Bowhead-Contract-ABI>`_ of this contract.
...
.. code-block:: javascript
    ...
    var contract1 = new aht.Contract(abi, address, {gasPrice: '12345678', from: fromAddress});
...
1. ``options`` - ``Object``: The options used for sending.
    ...
    * ``gasPrice`` - ``String`` (optional): The gas price in cell to use for this transaction.
    * ``gas`` - ``Number`` (optional): The maximum gas provided for this transaction (gas limit).
    * ``value`` - ``Number|String|BN|BigNumber``(optional): The value transferred for the transaction in cell.
...
1. ``options`` - ``Object`` (optional): The options used for calling.
    ...
    * ``value`` - ``Number|String|BN|BigNumber``(optional): The value transferred for the call "transaction" in cell.
