...
var StandAloneContract = require('../packages/web3-aht-contract');
...
var getEthContractInstance = function(abi, address, options, provider) {
    ...
    aht.setProvider(provider);
    return new aht.Contract(abi, address, options);
}
...
var runTests = function(contractFactory) {
    ...
    describe('with maht.ds', function () {
        ...
        it('should not throw error when trying to not send ahter to a non payable contract function', function (done) {
            var provider = new FakeIpcProvider();
...
describe('typical usage', function() {
    ...
    it('should update contract instance provider when assigned a provider to aht instance that contract instance came from', function () {
        var provider1 = new FakeIpcProvider();
        var provider2 = new FakeHttpProvider();

        var aht = new Aht(provider1);
        var contract = new aht.Contract(abi, address);
        assert.deepEqual(contract.currentProvider, provider1);
        assert.deepEqual(aht.currentProvider, provider1);

        aht.setProvider(provider2);
        assert.deepEqual(contract.currentProvider, provider2);
        assert.deepEqual(aht.currentProvider, provider2);
    });

    it('should deploy a contract, sign transaction, and return contract instance', function (done) {
        var provider = new FakeIpcProvider();
        var aht = new Aht(provider);
        aht.accounts.wallet.add(account.privateKey);

        provider.injectValidation(function (payload) {
            var expected = aht.accounts.wallet[0].signTransaction({
        ...
        var contract = new aht.Contract(abi);
