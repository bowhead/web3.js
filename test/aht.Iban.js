    ...
    describe('Iban', function () {
        tests.forEach(function (test) {
            it('toAddress() should transform iban to address: ' +  test.address, function () {
                if(test.error) {
                    assert.throws(aht.Iban.toAddress.bind(aht.Iban, test.direct));
                } else {
                    assert.deepEqual(aht.Iban.toAddress(test.direct), test.address);
                }
            });
            it('toIban() should transform address to iban: ' +  test.address, function () {
                if(test.error) {
                    assert.throws(aht.Iban.toIban.bind(aht. test.address));
                } else {
                    assert.deepEqual(aht.Iban.toIban(test.address), test.direct);
                }
            });
        });
    });
